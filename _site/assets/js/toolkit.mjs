...
export function createImage(width, height, filename, element, string, options = {}) {
    ...
    const lines = wrapTextByWidth(ctx, string, maxTextWidth);

    // Estimate line height from font size - adapted by CoPilot
    const metrics = ctx.measureText('M'); // Tall character for better accuracy
    const lineHeight = metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent;

    // Adjust font position if needed
    let textY = paddingTop;
    if (fontAdjust) {
        const totalTextHeight = lines.length * lineHeight;
        const adjustedY = adjustFontPosition(ctx, string, paddingTop, canvas.height);
        textY = Math.min(adjustedY, canvas.height - totalTextHeight);
    }

    let currentY = textY;
    lines.forEach((line) => {
        ctx.fillText(line, 10, currentY);
        currentY += lineHeight + 15; // Adding 15px for line spacing
    });
...
/**
 * Wraps a given text into multiple lines so that each line does not exceed the specified maximum width when rendered.
 * Full disclosure: Generated by CoPilot, as working in Canvas is a bit beyond my current knowledge.
 *
 * @param {CanvasRenderingContext2D} ctx - The canvas rendering context used to measure text width.
 * @param {string} text - The text to be wrapped.
 * @param {number} maxWidth - The maximum width (in pixels) allowed for each line.
 * @returns {string[]} An array of strings, each representing a line of wrapped text.
 */
