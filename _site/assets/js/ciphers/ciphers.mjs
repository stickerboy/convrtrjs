 ...
 */
function smithyEncrypt(string, key) {
 ...
 */
function smithyDecrypt(string, key) {
    const encoder = new TextEncoder();
    const encodedText = encoder.encode(string);
    const fib = fibonacci(key.length);
    const keywordShifts = key.split("").map((char, index) => (fib[index % fib.length] + char.charCodeAt(0)) % 26);
...
smithyEncryptButton && smithyEncryptButton.addEventListener("click", function () {
    ...
    document.getElementById("smithyResults").textContent = smithyEncrypt(kwResults, smithyKey.value);
});
...
smithyDecryptButton && smithyDecryptButton.addEventListener("click", function () {
    ...
    document.getElementById("smithyResults").textContent = smithyDecrypt(kwResults, smithyKey.value);
});
